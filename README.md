...
The client is published on [Packagist](https://packagist.org/packages/ondewo/survey-client-php) as
**`ondewo/survey-client-php`** and is installed like any other composer package:

```bash
composer require ondewo/survey-client-php
```

The client version tracks the ONDEWO SURVEY API in major and minor version, so pin the minor series that
matches the server you talk to:

```bash
composer require ondewo/survey-client-php:^2.0
```
...
* **`ext-grpc` is a hard requirement**, not an optional extra — every generated `<Service>Client`
  ...
  (see [Requirements](#requirements) above); `composer require ondewo/survey-client-php --ignore-platform-req=ext-grpc`
...
* **There is no build step.** Packagist serves the tree of a git tag verbatim and the generated stubs are
  ...
  `composer require`, `vendor/ondewo/survey-client-php/src/` already holds every stub and the autoloader is
  generated by composer itself. Nothing needs docker, protoc or the `ondewo-survey-api` submodule.
...
use Ondewo\Survey\SurveysClient;                   // generated, from src/
use Ondewo\Survey\Auth\BearerTokenAuthenticator;   // hand-written, from auth/
```
...
| `tests/Generated/MessageSerializationTest.php` | A field that never reaches the wire, a sub-message whose presence is lost, a moved enum zero constant, a broken JSON mapping (including the integer path, which needs `ext-bcmath`) |
...
1. **Submit the package once.** Log in to [packagist.org](https://packagist.org/) with the ONDEWO account and
   use *Submit* with the repository URL `https://github.com/ondewo/ondewo-survey-client-php`. Packagist reads
...
| `check_version_agreement` | `ONDEWO_SURVEY_VERSION`, `composer.json`'s `name` and `version`, the `RELEASE.md` entry and — on a tag — the tag name drifting apart. A tag that disagrees with the `version` field makes Packagist publish the tree under a version nobody tagged |
