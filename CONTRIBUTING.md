...
| `ondewo-survey-api`, `ondewo-proto-compiler` | their own repositories | as submodule commits |
...
1. Bump `ONDEWO_SURVEY_API_GIT_BRANCH` (and, for a new compiler release,
...
Releases are cut from the `Makefile`, never by hand: bump `ONDEWO_SURVEY_VERSION` (it must match the
ONDEWO SURVEY API in major and minor version), add a `RELEASE.md` entry under a
`## Release ONDEWO SURVEY PHP Client <version>` heading followed by the `*****` separator the release
